...
#define CPU_USAGE_READ      // Conditional compilation for single sample vs DMA mode
#define DMA
...
extern volatile uint32_t signalPeriod;
...
void displayFrequency(tContext *sContext);
...
void displayTask_func(UArg arg1, UArg arg2)
{
    ...
    while (true) {
        ...
        displayFrequency(&sContext);

        #ifdef CPU_USAGE_READ
...
void userInputTask_func(UArg arg1, UArg arg2)
{
    ...
    while(true){
        ...
        if(msg != 0){
            ...
            else if(msg == 'R'){    // Joystick right
                //decrease frequency
                pwmPeriod += 10;
                setPWMPeriod(pwmPeriod);
            }
            else if(msg == 'L'){    // Joystick left
                //increase frequency
                pwmPeriod -= 10;
                setPWMPeriod(pwmPeriod);
            }
            else if(msg == 'A'){    // User button 2
...
int risingTrigger(void)   // search for rising edge trigger
{
    ...
    #endif
    #ifdef DMA
        int32_t localBufferIndex = getADCBufferIndex();
    #endif
...
int32_t getADCBufferIndex(void)
{
    ...
    UInt gateKey = GateHwi_enter(gateHwi0);
...
void displayCPUuse(tContext *sContext){
    ...
    GrStringDrawCentered(sContext, str, /*length*/ -1, /*x*/ LCD_HORIZONTAL_MAX/2, /*y*/ 115, /*opaque*/ true);
...
void displayFrequency(tContext *sContext){
    float frequency = 1/(signalPeriod * 0.000000008333);

    char str[50];   // CPU usage string buffer
    GrContextForegroundSet(sContext, ClrWhite);
    GrContextBackgroundSet(sContext, ClrDarkSlateGray);

    snprintf(str, sizeof(str), "T=%d | f=%d Hz", signalPeriod, (int32_t)frequency); // convert time to string
    GrStringDrawCentered(sContext, str, /*length*/ -1, /*x*/ LCD_HORIZONTAL_MAX/2, /*y*/ 123, /*opaque*/ true);

    /*
    This function takes an argument of type tContext, which is a pointer to the display buffer used in the graphics drivers. It then calculates
    the current signal frequency usage, and writes it to the bottom of the display with a background.
    */
