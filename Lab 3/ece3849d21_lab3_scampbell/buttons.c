...
void ButtonInit(void)
{
    ...
    */
    ...
    //Boosterpack button 1 config, uses pin H1
    ...
    //Boosterpack button 2 config, uses pin K6
    ...
    //Boosterpack joystick button config, uses pin D4
...
void ButtonRoutine(void) {
    ...
    gpio_buttons |= (~GPIOPinRead(GPIO_PORTJ_BASE, 0xff) & (GPIO_PIN_1 | GPIO_PIN_0));   // EK-TM4C1294XL buttons in positions 0 and 1
    gpio_buttons |= ((~GPIOPinRead(GPIO_PORTH_BASE, 0xff) & GPIO_PIN_1) << 1);           // Boosterpack button 1 in position 2
    gpio_buttons |= ((~GPIOPinRead(GPIO_PORTK_BASE, 0xff) & GPIO_PIN_6) >> 3);           // Boosterpack button 2 in position 3
    gpio_buttons |= (~GPIOPinRead(GPIO_PORTD_BASE, 0xff) & GPIO_PIN_4);                 // Boosterpack joystick button in position 4
    ...
    if(presses & 0x100){ // Joystick down
        buttonMsg = 'D'; // ASCII D (down)
    }

    if(presses & 0x80){  // Joystick up
        buttonMsg = 'U'; // ASCII U (up)
    }

    if(presses & 0x10){  // Joystick press (select)
        buttonMsg = 'S'; // ASCII S (select)
    }

    if(presses & 0x4){   // User button 1
        buttonMsg = 'M'; // ASCII M (Scope mode)
    }

    if(presses & 0x20){  // Joystick right
        buttonMsg = 'R'; // ASCII R (Frequency increase)
    }

    if(presses & 0x40){  // Joystick left
        buttonMsg = 'L'; // ASCII L (Frequency decrease)
    }

    if(presses & 0x8){   // User button 2
        buttonMsg = 'A'; // ASCII A (Start audio)
    }

    if(buttonMsg != 0){
        Mailbox_post(buttonMailbox, &buttonMsg, BIOS_WAIT_FOREVER);
    }
